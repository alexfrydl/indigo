  ...
  fn from_spec(parse_ident: Ident, spec: ParserSpec) -> Sp<Self> {
    ...
    let func = match spec.parse_func {
      None => match kind {
        ...
        TryFromOsStr => abort!(spec.kind, "you must set parser for `try_from_os_str` explicitly"),
...
impl Attrs {
  fn new(
    ...
    let no_version = parent_attrs.as_ref().map(|attrs| attrs.no_version.clone()).unwrap_or(None);
  ...
  pub fn from_struct(
    ...
    if res.has_custom_parser {
      abort!(res.parser.span(), "`parse` attribute is only allowed on fields");
    }
    match &*res.kind {
  ...
  pub fn from_field(
    ...
    match &*res.kind {
      Kind::Flatten => {
        if res.has_custom_parser {
          abort!(res.parser.span(), "parse attribute is not allowed for flattened entry");
        }
        if res.has_explicit_methods() {
          abort!(res.kind.span(), "methods are not allowed for flattened entry");
        }
      ...
      Kind::Subcommand(_) => {
        if res.has_custom_parser {
          abort!(res.parser.span(), "parse attribute is not allowed for subcommand");
        }
        if res.has_explicit_methods() {
        ...
        match *ty {
          Ty::OptionOption => {
            abort!(field.ty, "Option<Option<T>> type is not allowed for subcommand");
          }
          Ty::OptionVec => {
            abort!(field.ty, "Option<Vec<T>> type is not allowed for subcommand");
          }
          _ => (),
        }
        ...
        match *ty {
          ...
          Ty::OptionOption => {
            if res.is_positional() {
              abort!(field.ty, "Option<Option<T>> type is meaningless for positional argument")
            }
          }
          Ty::OptionVec => {
            if res.is_positional() {
              abort!(field.ty, "Option<Vec<T>> type is meaningless for positional argument")
            }
          }
  ...
  fn set_kind(&mut self, kind: Sp<Kind>) {
    if let Kind::Arg(_) = *self.kind {
      ...
      abort!(kind.span(), "subcommand, flatten and skip cannot be used together");
    }
  }
  ...
  pub fn is_positional(&self) -> bool {
    self.methods.iter().all(|m| m.name != "long" && m.name != "short")
  }
  ...
  pub fn has_doc_methods(&self) -> bool {
    !self.doc_comment.is_empty()
      || self.methods.iter().any(|m| {
        m.name == "help" || m.name == "long_help" || m.name == "about" || m.name == "long_about"
      })
  }
}
