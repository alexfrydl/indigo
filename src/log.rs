...
#[cfg(feature = "logger")]
mod logger {
  pub use indigo_macros::log_init as init;

  use {
    super::*,
    crate::{
      prelude::*,
      sync::{blocking::RwLock, channel, ConcurrentHashMap, Lazy},
    },
    log_crate::LevelFilter,
    std::{
      cell::RefCell,
  ...
  pub fn init() {
    if log_crate::set_logger(&*LOGGER).is_err() {
      return;
    }
  ...
  fn write_message(time: Time, record: &log_crate::Record, f: &mut String) -> fmt::Result {
    use console::style;
