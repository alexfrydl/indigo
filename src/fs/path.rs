...
mod tests {
  ...
  fn test_append() {
    ...
    append(&mut p, "some/path");
    assert_eq!(p, "/some/path");

    append(&mut p, "/abs/olute");
  ...
  #[test]
  fn test_join() {
    assert_eq!(join("/", "some/path"), "/some/path");
    assert_eq!(join("/x/y", "/abs/olute"), "/abs/olute");
    assert_eq!(join("rel/ative", "some/path"), "rel/ative/some/path");
    assert_eq!(join("/a", ""), "/a/");
  }

  #[test]
  fn test_last() {
    assert_eq!(last("/a/b"), Some("b"));
    assert_eq!(last("/a"), Some("a"));
    assert_eq!(last("/"), None);
    assert_eq!(last("a"), Some("a"));
    assert_eq!(last(""), None);
  }
  ...
  fn test_normalized() {
    ...
    assert!(std::ptr::eq(normalized(&cow).as_ref(), cow.as_ref()));
  }
  ...
  fn test_pop() {
    ...
    assert_eq!(pop(&mut p), Some("b".into()));
    assert_eq!(p, "/a");
    assert_eq!(pop(&mut p), Some("a".into()));
    assert_eq!(p, "/");
  ...
  fn test_resolved() {
    ...
    assert_eq!(resolved("/a/../b").unwrap(), "/b");
    assert_eq!(resolved("/a/./b/").unwrap(), "/a/b");
    assert_eq!(resolved("a/../b/.").unwrap(), format!("{}/b", wd));
  }
}
